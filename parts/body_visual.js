...
function chemoVisualGetTumorMetrics(sample) {
	...
	var tumorShrinkFraction = typeof visualState.tumorShrinkFraction === "number" ? visualState.tumorShrinkFraction : 0;
...
function chemoVisualRenderBody() {
	...
	var svg = "" +
		...
		"<path d='M120 132 C138 108, 223 108, 240 132 L262 214 C268 236, 262 255, 248 270 L226 430 L194 430 L188 328 L172 328 L166 430 L134 430 L112 270 C98 255, 92 236, 98 214 Z' fill='rgba(31,42,51," + bodyOpacity.toFixed(2) + ")' stroke='" + outlineColor + "' stroke-width='2' />" +
		"<path d='M150 142 C162 132, 198 132, 210 142 C222 151, 228 174, 216 188 C204 202, 156 202, 144 188 C132 174, 138 151, 150 142 Z' fill='" + chemoVisualColor("bloodstream", visualState.bloodstream) + "' stroke='rgba(214,80,80,0.5)' />" +
	...
	healthBarRoot.innerHTML = "" +
		...
		"<div class='health-bar-label'><span>Patient vitality</span><strong>" + Math.round(patientHealth) + "%</strong></div>" +
		"<div class='health-bar-track'><div class='health-bar-fill' style='width:" + Math.max(0, Math.min(100, patientHealth)).toFixed(0) + "%; background:" + healthColor + ";'></div></div>" +
		"</div>";
	statusRoot.innerHTML = "" +
		"<div class='status-pill'><span>Bloodstream</span><strong>" + chemoVisualPercentText(visualState.bloodstream) + "</strong></div>" +
		...
		"<div class='status-pill'><span>Life status</span><strong>" + lifeStatus + "</strong></div>";
}
