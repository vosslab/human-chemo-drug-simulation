...
function chemoUiRenderMetrics() {
  ...
  // current burden
  document.getElementById("metric-total-burden").textContent =
    sample.totalBurden.toFixed(2) + " mg/L";
  // max burden (peak)
  document.getElementById("metric-peak-exposure").textContent =
  ...
  // response chance
  document.getElementById("metric-response-chance").textContent =
    Math.round(responseChance * 100) + "%";
  // patient health
  document.getElementById("metric-patient-vitality").textContent =
  ...
  document.getElementById("metric-weight").textContent =
    (patientProfile.weightKg || 0).toFixed(1) + " kg";
  document.getElementById("metric-bsa").textContent = (patientProfile.bsa || 0).toFixed(2) + " m2";
  document.getElementById("metric-clearance-multiplier").textContent =
    (patientProfile.clearanceMultiplier || 0).toFixed(2) + "x";
  document.getElementById("metric-resilience").textContent =
...
function chemoUiRenderSliderLabels() {
  ...
  document.getElementById("dose-multiplier-value").textContent =
    CHEMO_STATE.doseMultiplier.toFixed(2) + "x";
  document.getElementById("dose-count-value").textContent = CHEMO_STATE.doseCount;
  document.getElementById("dose-interval-value").textContent =
    CHEMO_STATE.doseIntervalDays.toFixed(0) + " d";
  document.getElementById("gender-value").textContent =
    CHEMO_STATE.genderBalance >= 0.5 ? "Male" : "Female";
  ...
  document.getElementById("activity-value").textContent =
    CHEMO_STATE.activityLevel >= 0.66
      ...
      : CHEMO_STATE.activityLevel >= 0.33
        ? "Moderate"
        : "Sedentary";
  document.getElementById("tumor-sensitivity-value").textContent =
    CHEMO_STATE.tumorSensitivity.toFixed(2) + "x";
  document.getElementById("playback-speed-value").textContent =
    CHEMO_STATE.playbackSpeed.toFixed(1) + "x";
...
function chemoUiRenderScrubberBounds() {
  ...
  var maxTimeHour = CHEMO_STATE.samples.length
    ? CHEMO_STATE.samples[CHEMO_STATE.samples.length - 1].timeHour
    : 0;
  var markup = [];
  var dayIndex;
